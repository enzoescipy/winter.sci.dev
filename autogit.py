        ...
        if arg == "commit":
            ...
            for follow in follows :
                if follow == "docker":
                    ...
                    tag = tag.stdout
                    tag = tag[:tag.find(" ")]
                    liveprint_exec(["autogit\\push_docker.bat", "ghcr.io/enzoescipy/winter-sci-dev:" + tag])
...
def fronter():
    ...
    print("    commit docker > build docker images. tag will be ghcr.io/enzoescipy/winter-sci-dev:pending")
    print("    commit git    > add current changes and commit with msg. will be applied to both main, react repos.")
    print("push : push sth.")
    print("    push docker   > push ghcr.io/enzoescipy/winter-sci-dev:pending. tag will be change to latest local git commit hashcode.")
