...
Consider the traditional Transformer models like BERT: each token's embedding carries a relatively low information density—a 'speck of dust' of meaning. Adding a positional signal to this dust doesn't significantly corrupt its original, sparse information.

However, in our vBERT system, each embedding vector is a 'jewel'—an information-dense compression of hundreds of tokens, carefully crafted by `intfloat/multilingual-e5-base`. When we attempted to apply traditional positional encoding to these 'jewels,' it was akin to tarnishing a finely cut gemstone by haphazardly smearing it with new information. The delicate balance was destroyed, and our benchmarks plummeted.
